...
describes what users get. Pick a format with `-o`, or let xianyu choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
xianyu <command> -o table   # aligned columns for reading
xianyu <command> -o jsonl   # one JSON object per line, for piping
xianyu <command> -o json    # a single JSON array
xianyu <command> -o csv     # spreadsheet friendly
xianyu <command> -o tsv     # tab-separated
xianyu <command> -o url     # just the URL column
xianyu <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
xianyu <command> --fields id,title,url
```
...
```bash
xianyu <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
xianyu <command>            # a table, because this is a terminal
xianyu <command> | wc -l    # JSONL, because this is a pipe
```
